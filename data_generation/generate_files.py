...
def two_raleigh_model_draw(num_draws, sigma1=1.4, sigma2=20, f=0.42):
    """
    Perform x draws from a two Raleigh distribution
    """
    # Decide which distribution each draw comes from
    choices = np.random.choice([0, 1], size=num_draws, p=[f, 1-f])
    
    # Draw from the appropriate Raleigh distribution
    draws = np.where(choices == 0,
                     np.random.rayleigh(sigma1, num_draws),
                     np.random.rayleigh(sigma2, num_draws))
    
    return draws
    

def sample_exoplanet_radii(n_samples):
    """
    Sample exoplanet radii with 60% from 2-8 R_Earth and 40% from 8-30 R_Earth
    """
    # Determine how many samples from each range
    n_small = int(0.6 * n_samples)
    n_large = n_samples - n_small

    #Sample from each range uniformly
    small_radii = np.random.uniform(2, 8, n_small)
    large_radii = np.random.uniform(8, 30, n_large)

    # Combine and shuffle
    all_radii = np.concatenate([small_radii, large_radii])
    np.random.shuffle(all_radii)
...
Earth_radius_in_km = 6371 # radius
AU_in_km = 1.496e8 # AU
...
# Transiting and non_transiting planet radii (we generate these numbers)
tran_planet_radii = sample_exoplanet_radii(num_test) * Earth_radius_in_km # in km
non_planet_radii = sample_exoplanet_radii(num_test) * Earth_radius_in_km # in km

# non-transiting planet inclinations drawn from Two Raleigh distribution

non_inclinations = two_raleigh_model_draw(num_test)
...
exoplanet_data = pd.read_csv(f"{cur_path}/exoplanet_rv_evidence_with_star_rad.csv", comment="#")

print(exoplanet_data['st_rad'])
# Get num_test number of random exoplanet systems
random_systems = exoplanet_data.sample(n=num_test, random_state=int(time.time())).reset_index(drop=True)

num_skipped =  0
for index, row in random_systems.iterrows():

    mass_star = row['st_mass']
    radius_star = row['st_rad']

    print(exoplanet_data['st_rad'])

    print("Orbital Period", row['pl_orbper'])

    calc_a_transit = (mass_star*(row['pl_orbper'] / DaysOfYear)**2)**(1/3) # AU
    a_transit = row['pl_orbsmax'] if not np.isnan(row['pl_orbsmax']) else calc_a_transit
    print(exoplanet_data['pl_orbsmax'])
    print('a_transit', a_transit, 'calc_a_transit', calc_a_transit)

    n_Rhill = 4 #np.random.uniform(3.5, 8.5)

    ############ randomly choose e ##################################
    e_transit = row['pl_orbeccen']
    ...
    #################### mass #############################
    mass_transit = row['pl_bmassj'] * Jupiter_in_Solar
    ...
    #################### a ################################
    # Get inner bound for non-transiting planet semi-major axis, and sample a from there to 5 AU uniformly
    a_nontransit_inner_bound = get_nontransit_a(mass_transit, mass_nontransit, mass_star, a_transit, n_Rhill, 'in', has_range)

    if math.isnan(a_nontransit_inner_bound):
        print('Cannot find a_nontransit_inner_bound, skip this system')
        num_skipped += 1
        continue
    print(a_nontransit_inner_bound, ((15/DaysOfYear)**2 * mass_star)**(1/3))
    ...
    # Get inclination lower bound for non-transiting planet
    incli_nontransit_bound = abs(np.arctan(radius_star*Solar_radius_in_km/(a_nontransit*AU_in_km)))

    # Assume transitting planet is perfectly edge-on, i.e., inclination = 0 (we are measuring mutual inclination)
    incli_transit = 0
    incli_nontransit = non_inclinations[index] * pi/180 + incli_nontransit_bound
    # if USE_2013:
    ...
    ################# Omega  #########################
    O_transit = np.random.uniform(0, 2*pi) 
    ...
    omega_transit = np.random.uniform(0, 2*pi)
    ...
    ###############  mean anomaly  #################
    mean_ano_transit = np.random.uniform(0, 2*pi)
    ...
    with open('file_names.txt', 'a') as file_name_write:
        file_name_write.write('num_'+str(current_num)+'_Test_' + str(index) + ' /mt_' + str(mass_transit) + '_mnont_' + str(mass_nontransit) # mass
    ...
    # print('JSON:')
    # print('{ "name":"transit", "rigid": false, "mass": '+ str(mass_transit)+', "orbital_elements": ['+ str(a_transit)+', '+ str(e_transit) +', '+ str(incli_transit) +', '+ str(O_transit) +', '+ str(omega_transit)+', ' + str(mean_ano_transit)+ '], "center_mass": '+ str(mass_star) +', "radius":' + str(tran_planet_radii[index]) + '},' + '\n') # radius doesn't matter as we use point mass
    # print('        { "name":"nontransit", "rigid": false, "mass": '+ str(mass_nontransit)+', "orbital_elements": ['+ str(a_nontransit)+', '+ str(e_nontransit) +', '+ str(incli_nontransit) +', '+ str(O_nontransit)+', ' + str(omega_nontransit) + ', ' + str(mean_ano_nontransit)+ '], "center_mass": '+ str(mass_star) +', "radius":' + str(non_planet_radii[index]) + '}' + '\n')
    # print('    "system_name":'+ ' "num_'+str(current_num)+'_Test_' + str(index) +'",' +'\n')
    # print('        { "name":"TSun", "rigid": false, "mass":'+ str(mass_star) +', "position": [0, 0, 0], "velocity": [0, 0, 0], "radius":'+str(radius_star*Solar_radius_in_km)+'},'+'\n')
    ...
    with open('../../Template/sample/Test_mass/init_system.json') as init_file: # modify the init file
        ...
        for i, line in enumerate(init_file):
            ...
            if i == 4:
                new_file.write('        { "name":"transit", "rigid": false, "mass": '+ str(mass_transit)+', "orbital_elements": ['+ str(a_transit)+', '+ str(e_transit) +', '+ str(incli_transit) +', '+ str(O_transit) +', '+ str(omega_transit)+', ' + str(mean_ano_transit)+ '], "center_mass": '+ str(mass_star) +', "radius":' + str(tran_planet_radii[index]) + '},' + '\n') # radius doesn't matter as we use point mass
            elif i == 5:
                new_file.write('        { "name":"nontransit", "rigid": false, "mass": '+ str(mass_nontransit)+', "orbital_elements": ['+ str(a_nontransit)+', '+ str(e_nontransit) +', '+ str(incli_nontransit) +', '+ str(O_nontransit)+', ' + str(omega_nontransit) + ', ' + str(mean_ano_nontransit)+ '], "center_mass": '+ str(mass_star) +', "radius":' + str(non_planet_radii[index]) + '}' + '\n')
            elif i == 1:
                new_file.write('    "system_name":'+ ' "num_'+str(current_num)+'_Test_' + str(index) +'",' +'\n')
            elif i == 3:
                ...
                new_file.write(line)
        new_file.close()
    print("Start simulation for num_"+str(current_num)+'_Test_' + str(index))
    os.system(simulate_exe_path+' ./')
    print("End simulation for num_"+str(current_num)+'_Test_' + str(index))
    ...
    current_txt='num_'+str(current_num)+'_Test_'+str(index)+'.txt'
